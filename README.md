...
- **`fetch_url`** renders any URL through crawl4ai as clean markdown — and dedicated engines return TOON instead: Reddit (threads *and* `/r/{sub}` listings — which honor the URL's own `?t=` time window and `?limit=` post count) through a real browser, plus Hacker News, GitHub issues / pull requests, and Discourse topics read straight from their public APIs.
