...
| `OMNIFEED_CRAWL4AI_SCAN_FULL_PAGE` | `false` | Scroll the full page before extraction so lazy-loaded content renders (crawl4ai `scan_full_page`). Costs multiple seconds on long pages — enable only if your corpus leans on infinite-scroll/lazy-loaded content. |
| `OMNIFEED_CRAWL4AI_SCROLL_DELAY` | `0.5` | Pause (seconds) between scroll steps while scanning the full page. Only sent when `OMNIFEED_CRAWL4AI_SCAN_FULL_PAGE=true`. |
