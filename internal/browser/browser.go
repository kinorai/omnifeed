...
// browser-only bot wall) drives a small interface instead of a concrete
// backend.
//
// One backend implements it: browser/crawl4ai drives crawl4ai's /execute_js
// endpoint. Navigate only records the target; each Eval re-navigates there
// (crawl4ai has no session reuse).
//
// The ports mirror domain's split: domain.Searcher answers "query → URLs",
